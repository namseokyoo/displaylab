...
export default function ViewingAngle() {
  ...
  const [activePreset, setActivePreset] = useLocalStorage<PresetType>(
    'displaylab::va::preset',
    'none',
  );
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver((entries) => {
      for (const entry of entries) {
        ...
        const size = Math.max(240, Math.min(550, w - 48));
        setChartSize(size);
      }
    });
  ...
  const loadPreset = useCallback(
    async (type: PresetType) => {
      ...
      setLoading(true);
      try {
        const fileName = type === 'oled' ? 'viewing-angle-oled.csv' : 'viewing-angle-lcd.csv';
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 overflow-x-hidden">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm">{t('viewing.subtitle')}</p>
      </div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <a
                ...
                download
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-md border border-gray-200 text-gray-500 hover:border-gray-400 dark:border-gray-700 dark:text-gray-400 dark:hover:border-gray-500 hover:text-gray-300 transition-colors"
              >
            ...
            {hasData && (
              <div>
                ...
                {comparisonMode && !hasComparison && (
                  <p className="text-xs text-gray-500 mt-1">{t('viewing.comparePrompt')}</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6" ref={chartContainerRef}>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 rounded-xl bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-800">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="flex min-h-11 min-w-11 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300"
                  title={t('common.downloadPng')}
                  aria-label={`${t('viewing.polarPlot')}: ${t('common.downloadPng')}`}
                >
            ...
            <div className="p-4 rounded-xl bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-800">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {t('viewing.colorShift')}
                </h2>
                <button
                  onClick={() => handleDownloadSvg(colorShiftRef, 'color-shift.png')}
                  className="flex min-h-11 min-w-11 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300"
                  title={t('common.downloadPng')}
                  aria-label={`${t('viewing.colorShift')}: ${t('common.downloadPng')}`}
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path
                      strokeLinecap="round"
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 rounded-xl bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-800">
              <div className="flex items-center justify-between mb-2">
                <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {t('viewing.deltaEDist')}
                </h2>
                <button
                  onClick={() => handleDownloadSvg(deltaERef, 'delta-e-heatmap.png')}
                  className="flex min-h-11 min-w-11 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:text-gray-500 dark:hover:bg-gray-800 dark:hover:text-gray-300"
                  title={t('common.downloadPng')}
                  aria-label={`${t('viewing.deltaEDist')}: ${t('common.downloadPng')}`}
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path
                      strokeLinecap="round"
