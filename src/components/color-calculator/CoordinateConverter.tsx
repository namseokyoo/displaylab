...
export default function CoordinateConverter() {
  ...
  return (
    <div className="p-6 rounded-xl bg-gray-900 border border-gray-800">
      <h2 className="text-lg font-semibold text-white mb-1">Coordinate Converter</h2>
      <p className="text-sm text-gray-400 mb-4">Bidirectional XYZ &harr; xyY conversion</p>
      ...
      {mode === 'xyz-to-xyY' ? (
        <>
          ...
          <div className="space-y-3 mb-4">
            <label className="block">
              ...
              <input
                ...
                className="mt-1 block w-full px-3 py-2 rounded-lg bg-gray-800 border border-gray-700 text-white text-sm focus:border-blue-500 focus:outline-none"
              />
            </label>
            <label className="block">
              <span className="text-xs text-gray-400">Y (Luminance)</span>
              <input
                type="number"
                step="any"
                value={xyzY}
          ...
          <div className="p-4 rounded-lg bg-gray-800/50 border border-gray-700/50">
            <div className="text-xs text-gray-400 mb-2">Result: xyY</div>
            {(() => {
              ...
              return (
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    ...
                    <div className="text-sm font-mono text-white">{result.x.toFixed(6)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500">y</div>
                    <div className="text-sm font-mono text-white">{result.y.toFixed(6)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500">Y</div>
                    <div className="text-sm font-mono text-white">{result.Y.toFixed(4)}</div>
                  </div>
                </div>
              );
            })()}
        ...
        <>
          ...
          {/* XYZ Result */}
          <div className="p-4 rounded-lg bg-gray-800/50 border border-gray-700/50">
            <div className="text-xs text-gray-400 mb-2">Result: XYZ</div>
            {(() => {
              ...
              return (
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <div className="text-xs text-gray-500">X</div>
                    <div className="text-sm font-mono text-white">{result.X.toFixed(4)}</div>
                  </div>
                  <div>
                    <div className="text-xs text-gray-500">Y</div>
                    <div className="text-sm font-mono text-white">{result.Y.toFixed(4)}</div>
                  ...
                  <div>
                    ...
                    <div className="text-sm font-mono text-white">{result.Z.toFixed(4)}</div>
                  </div>
                </div>
              );
            })()}
